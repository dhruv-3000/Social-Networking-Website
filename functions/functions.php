...
function insertPost(){
	if(isset($_POST['sub'])){
		...
		if(strlen($content) > 50000){
			//echo "<script>alert('Please Use 50000 or less than 50000 words!')</script>";
	...
	while($row_posts = mysqli_fetch_array($run_posts)){
		...
		if($content=="No" && strlen($upload_image) >= 1){
			...
			<div class='row'>
				...
				<div id='posts' class='col-sm-6' style='border: 5px solid #e6e6e6;
	padding: 40px 50px;
	box-shadow: 5px 5px 2px #888888;background-color: #e6e6e6;
					border-radius: 5px;'>
					<div class='row'>
  		...
  		<div class='row'>
  			...
  			<div class='panel panel-info'>
	  			<div class='panel-body'>
				  			 <from action='' method='post' class='form-inline'>
...
function search_user(){
	...
	if (isset($_GET['search_user_btn'])) {
		...
		$get_user = "SELECT * from users where f_name like '%$search_query%' AND l_name like '%$search_query%' OR user_name like '%$search_query%'";
	}
	else{
	...
	while ($row_user = mysqli_fetch_array($run_user)) {
		...
		echo "
				...
				<div class='col-sm-6'>
				  ...
				  <img src='users/$user_image' class='img-circle' width='100px' height='100px' title='$username' style='float:left;margin:1px;'/>
				  </a>
				  </div>
				  <div class='col-sm-6'>
